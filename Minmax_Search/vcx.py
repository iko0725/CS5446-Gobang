...
def findMax(self, player, score_):
    ...
    for i in range(self.height):
        for j in range(self.width):
            ...
            #     #                     (np.abs(i - lastMaxPoint[0]) == np.abs(j - lastMaxPoint[1]))):")
            s = self.AIScore[p[0]][p[1]
                                   ] if player == R.AI else self.oppScore[p[0]][p[1]]
            self.score[p] = s
            if s >= score_:
                # 这句话是什么意思？
                result.append(p)
    ...
    for i in range(self.height):
        for j in range(self.width):
            if self.board[i][j] == R.empty:
                ...
                # if player == R.AI else self.oppScore[p]
                s1 = self.oppScore[p]
...
def get_max(self, player, deep, totalDeep=0):
    ...
    if deep <= 0 or time.time() - self.startTime > config.vcxTimeLimit:
        return False
